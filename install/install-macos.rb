...
Dir.chdir CLI_PREFIX do
  system "/bin/bash -o pipefail -c $'/usr/bin/curl -fsSL --output bin/kube-cli.tar.gz $(/usr/bin/curl -fsSL #{CLI_URL} | /usr/bin/grep browser_download_url | /usr/bin/grep darwin_amd64.tar.gz | /usr/bin/cut -d \\\'\"\\\' -f 4)'"
